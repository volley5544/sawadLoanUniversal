...
**UI-only at this stage** — no backend/API wiring yet, no Firebase. Screens
render from mock data + a customer profile the host will provide. App
language/data is **Thai**; code comments are English.
...
- **Build web with `--pwa-strategy=none`** so the Flutter service worker doesn't
  ...
  `main.dart.js`.
- The web build output is `build/web/`. There's no hosting/deploy flow in this
  repo — the native host points its WebView at wherever this is served.
