...
/// [LoanPaymentDetailCardOld]s rendered as *siblings* underneath — which is how
/// the source lays it out, and why ยอดค้าง and ยอดปัจจุบัน read as two
/// separate blocks rather than one merged list.
class LoanPaymentOptionCardOld extends StatelessWidget {
  const LoanPaymentOptionCardOld({
    super.key,
    required this.label,
...
class LoanPaymentDetailCardOld extends StatelessWidget {
...
/// A label/value line inside a [LoanPaymentDetailCardOld].
class LoanPaymentDetailRowOld extends StatelessWidget {
  const LoanPaymentDetailRowOld({
    super.key,
    required this.label,
    required this.value,
  });
...
class LoanPaymentNoticeOld extends StatelessWidget {
  const LoanPaymentNoticeOld({super.key, required this.text, this.alert = false});
...
class LoanPaymentAmountFieldOld extends StatelessWidget {
  const LoanPaymentAmountFieldOld({
    super.key,
...
class LoanPaymentPrimaryButtonOld extends StatelessWidget {
  const LoanPaymentPrimaryButtonOld({
    super.key,
    required this.label,
...
/// this build uses.
void showLoanPaymentMessageOld(BuildContext context, String message) {
