...
/// ~~Base URL and credential for the top-up recalculation.~~ **Deleted
/// 2026-09-14** — `kTopupRecalApiBase`, `kTopupRecalApiAuth` and
/// `kTopupRecalConfigured` are gone with the test host they described.
///
/// The QA endpoint landed as **`POST /topup/recal` on the mobile API base**,
/// which is exactly the condition Outstanding #33 set for removing them: it is
/// HTTPS, it sends `access-control-allow-origin: *`, and it authenticates with
/// the customer's own bearer token — so there is no shared `Basic` service
/// account to ship, no cleartext-on-an-IP exception to keep, and no
/// `--dart-define` for a build to remember. See [TopupApi.fetchRecal].
///
/// ⚠ Two things in **other** repos were only there for the old host and should
/// come out with it: `http://34.142.213.42:8080/` in the srisawad app's
/// `_kHttpRequestAllowedPrefixes`, and the matching `<domain-config>` block in
/// `network_security_config.xml`. Both are on branch `pentest_resolved`.
