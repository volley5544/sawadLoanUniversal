...
///
/// This is the top-up amount screen as it stood before BA's redesign, still
/// reachable at `/topup/amount-old` so the two can be compared on a device. The live
/// screen is `topup_amount_page.dart`.
///
/// Nothing should be added here. When the new screen is signed off, delete
/// this file, its route and `TopupCardPageOld` together — a second copy that
...
///    [TopupFlow.outcome].
class TopupAmountPageOld extends StatefulWidget {
  const TopupAmountPageOld({super.key, required this.flow});

  final TopupFlow flow;

  @override
  State<TopupAmountPageOld> createState() =>
      _TopupAmountPageOldState();
}

class _TopupAmountPageOldState extends State<TopupAmountPageOld> {
