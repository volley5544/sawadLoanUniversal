...
/// ⚠ **Superseded — kept for reference only** (2026-09-12).
///
/// This is the top-up contract screen as it stood before BA's redesign, still
/// reachable at `/topup/old` so the two can be compared on a device. The live
/// screen is `topup_card_page.dart`.
///
/// Nothing should be added here. When the new screen is signed off, delete
/// this file, its route and `TopupAmountPageOld` together — a second copy that
/// nobody maintains is worse than no copy, because it is indistinguishable
/// from the real one in a grep.
...
/// to one card.
class TopupCardPageOld extends StatefulWidget {
  const TopupCardPageOld({
    super.key,
    this.source = '',
    this.referId = '',
    this.contractNo = '',
  ...
  @override
  State<TopupCardPageOld> createState() => _TopupCardPageOldState();
}

class _TopupCardPageOldState extends State<TopupCardPageOld> {
  ...
  void _start(LoanContract contract, {LoanProduct? product}) {
    ...
    context.push(AppRoutes.topupAmountOld, extra: flow);
  }
